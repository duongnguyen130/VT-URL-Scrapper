...
function armIdleQuips() {
  // Nothing has moved for a while, so there is no fact left to show.
  ...
  try {
    const res = await fetch("/api/scan", {
      ...
      body: JSON.stringify({
        ...
        workers: Number(el.workers.value) || 1,
      }),
    });
    ...
    while (true) {
      ...
      while ((nl = buffer.indexOf("\n")) >= 0) {
        ...
        if (msg.event === "start") {
          total = msg.total;
          el.runCount.textContent = `0 / ${total}`;
        } else if (msg.event === "progress") {
          // Several lookups are in flight at once, so this line reports the
          // most recently started one rather than "the" current URL.
          showFact(msg.url);
          armIdleQuips();
...
function insertCard(res) {
  // Workers finish out of order. Slotting each card in by index keeps the
  // list in the order the URLs were pasted, which is what export and any
  // screenshot will match.
  const card = buildCard(res);
  card.dataset.index = res.index;

  const after = Array.from(el.rows.children).find(
    (c) => Number(c.dataset.index) > res.index
  );
  el.rows.insertBefore(card, after || null);
}
